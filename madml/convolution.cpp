...
namespace kernel {
	namespace layers {
		...
		bool convolution::forward(tensor& x, tensor& y, tensor& z) {
			...
			switch (dim) {
			case 1:
				input_x = x.getShape().at(x.size()-1);
				output_x = input_x + 2 * padding_x - dialation_x * (kernel_size_x - 1) - 1;
				output_x = output_x / stride_x + 1;
				param = {1, input_x, 0, 0, kernel_size_x, 0, 0, stride_x, 0, 0, dialation_x, 1, 1, padding_x, 0, 0, output_x, 0, 0, padding_type};
				break;
			case 2:
				input_x = x.getShape().at(x.size() - 1);
				input_y = x.getShape().at(x.size() - 2);
				output_x = input_x + 2 * padding_x - dialation_x * (kernel_size_x - 1) - 1;
				output_x = output_x / stride_x + 1;
				output_y = input_y + 2 * padding_y - dialation_y * (kernel_size_y - 1) - 1;
				output_y = output_y / stride_y + 1;
				param = { 2, input_x, input_y, 0, kernel_size_x, kernel_size_y, 0, stride_x, stride_y, 0, dialation_x, dialation_y, 1, padding_x, padding_y, 0, output_x, output_y, 0, padding_type };
				break;
			case 3:
				input_x = x.getShape().at(x.size() - 1);
				input_y = x.getShape().at(x.size() - 2);
				input_z = x.getShape().at(x.size() - 3);
				output_x = input_x + 2 * padding_x - dialation_x * (kernel_size_x - 1) - 1;
				output_x = output_x / stride_x + 1;
				output_y = input_y + 2 * padding_y - dialation_y * (kernel_size_y - 1) - 1;
				output_y = output_y / stride_y + 1;
				...
				param = { 3, input_x, input_y, input_z, kernel_size_x, kernel_size_y, kernel_size_z, stride_x, stride_y, stride_z, dialation_x, dialation_y, dialation_z, padding_x, padding_y, padding_z, output_x, output_y, output_z, padding_type };
				break;
			}
		...
		bool convolution::computeGroupCount() {
			m_group_x = (int)alignSize(1024, LOCAL_SZ_X) / LOCAL_SZ_X;
