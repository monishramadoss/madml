...
class L1Loss(_Loss):
    ...
    def forward_cpu(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        m = input.shape[0]
        data_loss = np.sum(np.abs(target - input)) / m
        self.cache = [input, target, m]
        return data_loss

    def backward_cpu(self) -> np.ndarray:
        input, target, m = self.cache
        grad_y = np.sign(input - target.reshape(-1, 1))
        grad_y /= m
        return grad_y
...
class MSELoss(_Loss):
    ...
    def forward_cpu(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        m = input.shape[0]
        data_loss = (np.square(input - target)).mean(axis=0)
        data_loss /= m
        self.cache = [input, target, m]
        return data_loss
    def backward_cpu(self) -> np.ndarray:
        input, target, m = self.cache
        grad_y = 2 * (input - target)
        grad_y /= m
        return grad_y
...
class HingeLoss(_Loss):
    ...
    def forward_cpu(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        ...
        margin = (input.T - input[range(m), target]).T
        ...
        self.cache = [input, target, m, margin]
        return data_loss #+ reg_loss

    def backward_cpu(self) -> np.ndarray:
        input, target, m, margins = self.cache
        margins[range(m), target] = 0
...
class CrossEntropyLoss(_WeightedLoss):
    ...
    def forward_cpu(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        ...
        self.cache = [input, target, prob, m]
        return data_loss  #+ reg_loss

    def backward_cpu(self) -> np.ndarray:
        input, target, grad_y, m = self.cache
