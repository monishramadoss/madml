...
namespace layers
{
	...
	{
		return this->operator()(_x, _w, std::make_shared<tensor>(tensor(0., std::vector<int>{1})));
	}
	std::shared_ptr<tensor>& gemm::operator()(const std::shared_ptr<tensor>& _x, const std::shared_ptr<tensor>& _w, std::shared_ptr<tensor>& _b)
	{
